...
namespace gpmf_to_yaml
{
  ...
  class converter
  {
    public:
      ...
      int32_t init(const std::string& in, const std::string& out_dir,float fr); //init parsing changing parameters
      int32_t cleanup(); //cleanup and exit
      int32_t run(); //run conversion
    ...
    private:
      ...
      int32_t sensorframes_to_yaml(); // output desired yaml
